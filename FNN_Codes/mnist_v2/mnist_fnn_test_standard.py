...
def my_kaiming_uniform(tensor, nonlinearity='leaky_relu', use_dim0=False, a=0):
    if use_dim0:
        dim = tensor.shape[0]
    else:
        dim = tensor.shape[1]
...
class Linear_with_param_adapt_v0(nn.Module):
    def __init__(self, dim_in, dim_out, bias=True):
        super(Linear_with_param_adapt, self).__init__()
        self.use_bias = bias
        self.dim_in = dim_in
        self.dim_out = dim_out
        self.weight = nn.Parameter(torch.rand(self.dim_in, self.dim_out))
        my_kaiming_uniform(self.weight, a=math.sqrt(5))
        if self.use_bias:
    ...
    def forward(self, x, adapt_vector=None):
        if adapt_vector is not None:
            ...
            weight = self.weight
        code_verion = 0 # both 0 and 1 are OK
        if code_verion == 0:
            W_times_x = torch.mm(x, weight)
            if self.use_bias:
...
class FnnNet7(nn.Module):
    def __init__(self):
        super(FnnNet7, self).__init__()
        self.fc = Linear_with_param_adapt(28*28, 10, bias=True)
        #self.fc = Linear_with_param_adapt(28*28, 10, bias=False)
